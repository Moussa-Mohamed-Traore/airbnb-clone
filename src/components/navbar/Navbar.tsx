...
import { useCreateListingModal } from "@/store/useCreatingListingModal";
...
export default function Navbar() {
  ...
  const {open: openCreatingListing} = useCreateListingModal();
  ...
  return (
    <nav className="fixed top-0 z-50 w-full h-18 lg:h-24 bg-white border-b border-gray-200">
      <div className="flex items-center justify-between h-full mx-auto w-[95%] md:[90%] ">
        <Logo />
        ...
        <div className="flex items-center gap-4 relative" ref={menuRef}>
          {session  && !isPending && (
            <button onClick={openCreatingListing} className="hidden md:block text-sm font-medium mx-4 px-4 py-2 rounded-full bg-gray-50 cursor-pointer hover:bg-gray-100">
              Airbnb your home
            </button>
          )}
          ...
          {open && (
            <div className="absolute right-0 top-14 w-56 bg-white border border-gray-200 rounded-xl shadow-lg overflow-hidden px-4 py-2">
              <ul className="text-gray-800 text-sm">
                {session && !isPending && (
                  <>
                    <li onClick={openCreatingListing} className="px-4 py-3 hover:bg-gray-100 cursor-pointer">
                      Airbnb your home
                    </li>
                    <Link href="/favorites">
